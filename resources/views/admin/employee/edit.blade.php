                    ...
                    <form action="{{route('employee.update',$employee[0]->id)}}" method="post">
                        ...
                        <div class="card-body">
                            <div>
                                ...
                                <div>
                                    ...
                                    <select name="company_name" id="defaultFormControlInput" class="form-select">
                                        ...
                                        @foreach ($companies as $row)
                                            <option value="{{ $row->companies_name }}" {{ ( $row->companies_name == $employee[0]->company_name) ? 'selected' : '' }}>{{$row->companies_name}}</option>
                                        @endforeach
                                    </select>
                                    @error('company_name')
                                ...
                                <input type="text" name="employee_name" class="form-control" id="defaultFormControlInput1" placeholder="Employee Name" aria-describedby="defaultFormControlHelp" value={{ old('employee_name', $employee[0]->employee_name) }} >
